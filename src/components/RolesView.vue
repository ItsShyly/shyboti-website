...
<template>
  <div class="roles">
    <div class="roles-header">
      ...
      <span v-if="saved" class="autosave-indicator">{{
        t("roles.saved")
      }}</span>
    </div>
    ...
    <div class="section">
      ...
      <div v-else class="mod-list">
        <div
          ...
          class="mod-item"
          :class="{
            expanded: expandedMod === mod.username,
            blocked: mod.blocked,
            overridden: !mod.blocked && mod.permissions !== null,
          }"
        >
          <!-- Mod header row -->
          <div class="mod-header" @click="toggleExpand(mod.username)">
            <span
              ...
              :class="{
                blocked: mod.blocked,
                override: !mod.blocked && mod.permissions !== null,
              }"
            ></span>
            <span class="mod-name">{{ mod.username }}</span>
            <span class="mod-badge" v-if="mod.blocked">{{
              t("roles.badge.blocked")
            }}</span>
            <span
              class="mod-badge override"
              v-else-if="mod.permissions !== null"
              >{{ t("roles.badge.custom") }}</span
            >
            <span class="mod-badge default" v-else>{{
              t("roles.badge.default")
            }}</span>
            <div class="mod-header-actions" @click.stop>
              <button
                class="mod-block-btn"
                :class="{ 'mod-block-btn-allow': mod.blocked }"
                @click="blockMod(mod)"
                :disabled="modSaving === mod.username"
              >
                {{ mod.blocked ? t("roles.unblock") : t("roles.block") }}
              </button>
            </div>
            <span class="mod-chevron">{{
          ...
          <div
            v-if="expandedMod === mod.username && !mod.blocked"
            class="mod-perms"
          >
            <div class="mod-perms-header">
              <span class="mod-perms-sub">{{
                mod.permissions !== null
                  ? t("roles.custom_perms")
                  : t("roles.using_default")
              }}</span>
              <button
                v-if="mod.permissions !== null"
                class="reset-btn"
                @click="clearModOverride(mod)"
                :disabled="modSaving === mod.username"
              >
                {{ t("roles.reset") }}
              </button>
              <button v-else class="override-btn" @click="setModOverride(mod)">
                {{ t("roles.set_custom") }}
              </button>
            </div>
                ...
                <div class="perm-rows">
                  ...
                  >
                    ...
                    <div class="toggle-wrap">
                      <span
                        class="default-val"
                        :class="{ active: (globalPerms as any)[perm.key] }"
                      >
                        {{
                          (globalPerms as any)[perm.key]
                            ? t("roles.default_on")
                            : t("roles.default_off")
                        }}
                      </span>
                ...
                <div class="perm-rows">
                  ...
                  >
                    ...
                    <div
                      class="toggle sm"
                      :class="{
                        on: modsEnabled && (globalPerms as any)[perm.key],
                      }"
                    >
                      <div class="toggle-knob"></div>
                    </div>
