...
function openNew() {
  ...
  editTrigger.value = { name: '', event_type: 'message', match_pattern: '', match_type: 'contains',
    ...
    required_game: '', condition: '', cooldown_sec: 30, is_active: 1 }
  editOpen.value = true
  setTimeout(() => { if (editorRef.value) { editorRef.value.innerText = ''; applyScriptHighlight(editorRef.value) } }, 50)
}
...
function openEdit(trigger: Trigger) {
  ...
  editOpen.value = true
  setTimeout(() => { if (editorRef.value) { editorRef.value.innerText = trigger.response; applyScriptHighlight(editorRef.value) } }, 50)
}
...
<template>
  <div class="ep-view">
    <div class="ep-view-header">
      <div class="view-header-left">
        <div>
          ...
          <div class="ep-view-sub">{{ t('trigger.sub') }} #{{ session?.channel }}</div>
          <button v-if="syncConf?.is_active" class="ep-sync-indicator" @click="syncOpen = !syncOpen" :title="`${t('trigger.sync.active')} #${syncConf.sync_from}`">
            <span class="ep-sync-dot"></span>{{ t('trigger.sync.active') }} #{{ syncConf.sync_from }}
            <span class="ep-sync-chevron">{{ syncOpen ? '▲' : '▼' }}</span>
          </button>
          <button v-else class="ep-sync-config-btn" @click="syncOpen = !syncOpen">{{ t('trigger.sync.config') }}<span class="ep-sync-chevron">{{ syncOpen ? '▲' : '▼' }}</span></button>
        </div>
      </div>
      <button class="ep-btn-new" @click="canEdit && openNew()" :disabled="!canEdit">{{ t('trigger.new') }}</button>
    </div>
    ...
    <div v-if="syncOpen" class="ep-sync-panel">
      <div class="ep-sync-row">
        ...
        <button class="ep-sync-save-btn" @click="saveSync" :disabled="syncSaving || !syncFrom">{{ syncSaving ? '…' : syncConf?.is_active ? t('trigger.sync.update') : t('trigger.sync.enable') }}</button>
        <button v-if="syncConf?.is_active" class="ep-sync-stop-btn" @click="stopSync">{{ t('trigger.sync.stop') }}</button>
      </div>
    ...
    <div v-else-if="!triggers.length" class="ep-empty">{{ t('trigger.empty') }}</div>

    <div v-else class="ep-row-list">
      <div v-for="trigger in triggers" :key="trigger.id" class="ep-list-row trigger-row" :class="{ inactive: !trigger.is_active }">
        <div class="trigger-toggle-wrap">
          <button class="ep-toggle-btn" :class="{ on: trigger.is_active, disabled: !canToggle }" @click="canToggle && toggleActive(trigger)">
        ...
        <div class="trigger-info" @click="openEdit(trigger)">
          ...
          <div class="trigger-meta">
            <span class="ep-meta-pill event">{{ eventLabel(trigger.event_type) }}</span>
            <span v-if="trigger.match_pattern" class="ep-meta-pill pattern">{{ matchLabel(trigger.match_type) }}: "{{ trigger.match_pattern.slice(0,20) }}{{ trigger.match_pattern.length>20?'…':'' }}"</span>
        ...
        <div class="ep-row-actions">
          <button class="ep-btn-action edit" @click.stop="canEdit && openEdit(trigger)" :class="{ disabled: !canEdit }">{{ canEdit ? t('trigger.edit') : t('trigger.view') }}</button>
          <button class="ep-btn-action share" @click.stop="openShare(trigger.name)" title="Copy to another channel">↪</button>
          <button v-if="canDelete" class="ep-btn-action del" @click.stop="deleteTrigger(trigger.name)" :disabled="saving === trigger.name">✕</button>
        </div>
      </div>
    </div>
        ...
        <div class="ep-panel">
          ...
          <div class="ep-panel-body">
            ...
            <div v-if="isNew" class="ep-field-group">
              <label class="ep-field-label">{{ t('trigger.field.name') }}</label>
              <input v-model="editTrigger.name" class="ep-field-input" placeholder="hype-train" />
            </div>

            <!-- Event type -->
            <div class="ep-field-group">
              <label class="ep-field-label">{{ t('trigger.field.event') }}</label>
            ...
            <div v-if="needsPattern(editTrigger.event_type ?? 'message')" class="ep-field-group">
              <label class="ep-field-label">{{ t('trigger.field.match') }}</label>
              <div class="match-row">
            ...
            <!-- Action type -->
            <div class="ep-field-group">
              <label class="ep-field-label">{{ t('trigger.field.action') }}</label>
            ...
            <div class="ep-field-group">
              <label class="ep-field-label">
                ...
                <span class="ep-field-hint">{{ t('trigger.field.resp_hint') }}</span>
              </label>
              <div
                ref="editorRef"
            ...
            <div class="ep-row-3">
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('trigger.field.active_when') }}</label>
              ...
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('trigger.field.game') }}</label>
                <input v-model="editTrigger.required_game" class="ep-field-input" placeholder="optional" />
              </div>
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('trigger.field.cd') }} <span class="ep-field-hint">s</span></label>
                <input v-model.number="editTrigger.cooldown_sec" type="number" min="0" class="ep-field-input" />
              </div>
            </div>

            <div class="ep-field-group">
              <label class="ep-field-label">{{ t('trigger.field.condition') }} <span class="ep-field-hint">{{ t('trigger.field.cond_hint') }}</span></label>
              <input v-model="editTrigger.condition" class="ep-field-input ep-mono" placeholder="$channel.game == Just Chatting" />
            </div>

            <div class="ep-panel-footer">
              <button v-if="!isNew && canDelete" class="ep-btn-delete" @click="deleteTrigger(editOrigName); editOpen = false">{{ t('trigger.delete') }}</button>
              <div v-else></div>
              <div class="ep-footer-right">
                <button class="ep-btn-cancel" @click="editOpen = false">{{ t('trigger.cancel') }}</button>
  ...
  <Teleport to="body">
    <div v-if="shareOpen" class="ep-modal-overlay" @click.self="shareOpen = false">
      <div class="ep-modal">
        <div class="ep-modal-title">{{ t('trigger.share.title') }} <span class="ep-modal-name">{{ shareTrigger }}</span></div>
        ...
        <div class="ep-modal-footer">
          <button class="ep-btn-cancel" @click="shareOpen = false">{{ t('trigger.cancel') }}</button>
          <button class="ep-btn-save" @click="doShare" :disabled="shareSaving || !shareTarget">{{ shareSaving ? t('trigger.share.copying') : t('trigger.share.btn') }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
.trigger-toggle-wrap { flex-shrink: 0; }
...
.ep-meta-pill.event   { color: #569cd6; border-color: #569cd644; background: #569cd611; }
.ep-meta-pill.pattern { color: #e5c07b; border-color: #e5c07b44; background: #e5c07b11; font-family: monospace; }
.ep-meta-pill.action  { color: #23d18b; border-color: #23d18b44; background: #23d18b11; }
.ep-meta-pill.when    { color: #e5c07b; border-color: #e5c07b44; background: #e5c07b11; }
.ep-meta-pill.game    { color: #4ec9b0; border-color: #4ec9b044; background: #4ec9b011; }
.ep-meta-pill.cd      { color: #c792ea; border-color: #c792ea44; background: #c792ea11; }

.event-grid, .action-grid { display: flex; flex-wrap: wrap; gap: 4px; }
.event-btn, .action-btn { height: 28px; padding: 0 12px; border: 1px solid #2a2a30; background: #111217; color: #666; font-family: inherit; font-size: 11px; cursor: pointer; transition: all .15s; }
.event-btn:hover, .action-btn:hover { color: #aaa; border-color: #444; }
...
.match-row .ep-field-input { flex: 1; }
...
@media (max-width: 680px) {
  ...
  .event-grid, .action-btn, .action-grid { flex-wrap: wrap; }
