...
function openEdit(w: Widget) {
  ...
  const style = (() => { try { return JSON.parse(w.style) } catch { return {} } })()
  form.value = {
    name:       w.name,
    content:    w.content,
...
async function previewContent() {
  ...
  try {
    // >>> Save a temp widget or use a dedicated preview endpoint
    // >>> For now: save the widget first then hit /data
    ...
    if (res.ok) {
      ...
      if (r2.ok) {
        const d2 = await r2.json() as { value: string }
        previewValue.value = d2.value || '(empty)'
      }
    }
  } catch {}
...
const SCRIPT_HINTS = [
  { label: 'Counter value',      example: '$counter.myCounter.get' },
  { label: 'Variable',           example: '$var.myVar' },
  { label: 'Channel viewers',    example: '$channel.viewers' },
  { label: 'Stream game',        example: '$channel.game' },
  { label: 'Stream title',       example: '$channel.title' },
  { label: 'Stream uptime',      example: '$channel.uptime' },
  { label: 'Math / calc',        example: '$calc($counter.kills.get / $counter.games.get)' },
  { label: 'Static text',        example: 'Hello stream!' },
]
      ...
      <div v-for="w in widgets" :key="w.id" class="widget-row">
        ...
        <div class="widget-actions">
          ...
          <div class="row-btns">
            ...
            <button class="btn-action del" :class="{ confirm: deleteId === w.id }" @click="deleteWidget(w.id, w.name)">
              {{ deleteId === w.id ? 'Sure?' : '✕' }}
            </button>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div class="panel-body">
            ...
            <div class="field-group">
              <label class="field-label">Widget name <span class="field-hint">internal label only</span></label>
            ...
            <div class="field-group">
              ...
              <div class="hints-grid">
                <button
                  v-for="h in SCRIPT_HINTS" :key="h.example"
                  class="hint-btn"
                  @click="form.content = h.example"
                >{{ h.label }}</button>
              </div>
            </div>
            ...
            <div class="preview-section">
              ...
              <div class="preview-box" :style="{ background: form.style.background || '#1a1a1e' }">
            ...
            <!-- Refresh -->
            <div class="field-group">
              <label class="field-label">Refresh every <span class="field-hint">milliseconds</span></label>
              <div class="refresh-row">
                ...
                <input v-model.number="form.refresh_ms" type="number" min="500" class="field-input ms-custom" placeholder="ms" />
              </div>
            </div>
            ...
            <div class="style-section">
              ...
              <div class="field-group">
                <label class="field-label">Background <span class="field-hint">leave empty for transparent</span></label>
                <div class="color-row">
...
<style scoped>
.obs-view { display: flex; flex-direction: column; gap: 16px; height: 100%; }
.obs-header { display: flex; align-items: flex-start; justify-content: space-between; }
.obs-title  { font-size: 18px; font-weight: 700; color: #e0e0e0; margin-bottom: 4px; }
.obs-sub    { font-size: 12px; color: #555; }
...
.toast.ok { padding: 8px 14px; font-size: 12px; background: rgba(35,209,139,.1); border: 1px solid rgba(35,209,139,.3); color: #23d18b; }
.empty { color: #444; font-size: 13px; padding: 40px; text-align: center; line-height: 1.8; }
.empty-hint { color: #333; font-size: 11px; }

/* >>> Widget list */
.widget-list { display: flex; flex-direction: column; gap: 2px; overflow-y: auto; flex: 1; }
.widget-row  {
  display: flex; align-items: center; gap: 12px;
  padding: 12px 14px; background: #141418; border-bottom: 1px solid #1e1e1e;
  transition: background .1s;
}
.widget-row:hover { background: #1c1c20; }
...
.btn-action { height: 26px; padding: 0 10px; border: 1px solid; background: transparent; font-family: inherit; font-size: 10px; cursor: pointer; transition: background .1s; }
.btn-action.edit { border-color: #6f2bff66; color: #9d6cff; }
.btn-action.edit:hover { background: #6f2bff18; }
...
.field-select { background: #111217; border: 1px solid #2a2a30; color: #e0e0e0; font-family: inherit; font-size: 13px; padding: 7px 10px; outline: none; appearance: none; cursor: pointer; }
.field-select:focus { border-color: #6f2bff55; }

.hints-grid { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 4px; }
.hint-btn { height: 22px; padding: 0 9px; border: 1px solid #2a2a30; background: transparent; color: #666; font-family: inherit; font-size: 10px; cursor: pointer; transition: all .1s; white-space: nowrap; }
...
.preview-box { min-height: 60px; padding: 12px; display: flex; align-items: center; border: 1px solid #2a2a30; }
