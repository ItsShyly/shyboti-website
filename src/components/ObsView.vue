      ...
      <div v-for="w in widgets" :key="w.id" class="widget-row">
        ...
        <div class="widget-actions">
          ...
          <div class="row-btns">
            ...
            <button class="btn-action del" :class="{ confirm: deleteId === w.id }" @click="deleteWidget(w.id, w.name)">{{ deleteId === w.id ? 'Sure?' : '✕' }}</button>
          </div>
        </div>
      </div>
    </div>
...
.widget-row     { display: flex; align-items: center; gap: 12px; padding: 12px 14px; background: #141418; border-bottom: 1px solid #1e1e1e; transition: background .1s; }
...
.btn-action     { height: 26px; padding: 0 10px; border: 1px solid; background: transparent; font-family: inherit; font-size: 10px; cursor: pointer; }
