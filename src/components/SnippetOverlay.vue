...
onMounted(() => {
  watch(screenshotDrag, (v) =>
    document.body.classList.toggle("snippet-dragging", v),
  );
      ...
      <div v-if="screenshotToast" class="snippet-toast-global">
        ...
        <template v-else>
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            class="snippet-toast-img-link"
          >
            <div
              v-if="!screenshotToast.imgReady"
              class="snippet-toast-img-placeholder"
            >
...
.snippet-toast-spinner {
  ...
  border-radius: 50%;
  animation: snippet-spin 0.8s linear infinite;
  flex-shrink: 0;
}
.snippet-toast-spinner.small {
  width: 20px;
  height: 20px;
  border-width: 2px;
}
@keyframes snippet-spin {
  to {
...
.snippet-toast-label.copied {
  color: #9d6cff;
}
.snippet-toast-img-link {
  display: block;
}
...
.snippet-toast-img.hidden {
  display: none;
}
.snippet-toast-img:hover {
  opacity: 0.85;
}
.snippet-toast-fade-enter-active,
.snippet-toast-fade-leave-active {
  transition:
    opacity 0.25s,
    transform 0.25s;
}
.snippet-toast-fade-enter-from,
.snippet-toast-fade-leave-to {
  opacity: 0;
