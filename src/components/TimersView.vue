...
const searchOpenTimer = inject<Ref<string | null>>('searchOpenTimer', ref(null))
watch(searchOpenTimer, (name) => {
  if (!name) return
  searchOpenTimer.value = null
  const timer = timers.value.find(ti => ti.name === name)
  if (timer) { nextTick(() => openEdit(timer)) }
  else { load().then(() => { const t2 = timers.value.find(ti => ti.name === name); if (t2) openEdit(t2) }) }
})
...
function openEdit(timer: Timer) {
  ...
  editOpen.value = true
  setTimeout(() => {
    if (editorRef.value) { editorRef.value.innerText = timer.response; applyScriptHighlight(editorRef.value) }
  }, 50)
}
...
<template>
  <div class="ep-view">
    <div class="ep-view-header">
      <div class="view-header-left">
        <div>
          ...
          <div class="ep-view-sub">{{ t('timer.sub') }} #{{ session?.channel }}</div>
    ...
    <div v-if="syncOpen" class="ep-sync-panel">
      <div class="ep-sync-row">
        ...
        <button class="ep-sync-save-btn" @click="saveSync" :disabled="syncSaving || !syncFrom">{{ syncSaving ? '…' : syncConf?.is_active ? t('timer.sync.update') : t('timer.sync.enable') }}</button>
        <button v-if="syncConf?.is_active" class="ep-sync-stop-btn" @click="stopSync">{{ t('timer.sync.stop') }}</button>
      </div>
    ...
    <div v-else-if="!timers.length" class="ep-empty">{{ t('timer.empty') }}</div>

    <div v-else class="ep-row-list">
      <div v-for="timer in timers" :key="timer.id" class="ep-list-row timer-row" :class="{ inactive: !timer.is_active }">
        <div class="timer-toggle-wrap">
          <button class="ep-toggle-btn" :class="{ on: timer.is_active, disabled: !canToggle }" @click="canToggle && toggleActive(timer)" :title="timer.is_active ? 'Disable' : 'Enable'">
        ...
        <div class="ep-row-actions">
          <button class="ep-btn-action edit" @click.stop="canEdit && openEdit(timer)" :class="{ disabled: !canEdit }">{{ canEdit ? t('timer.edit') : t('timer.view') }}</button>
          <button class="ep-btn-action share" @click.stop="openShare(timer.name)" :title="t('timer.share')">↪</button>
          <button v-if="canDelete" class="ep-btn-action del" @click.stop="deleteTimer(timer.name)" :disabled="saving === timer.name">✕</button>
        </div>
      </div>
    </div>
      ...
      <div v-if="editOpen" class="ep-overlay" v-bind="overlay.handlers(() => editOpen = false)">
        <div class="ep-panel">
          ...
          <div class="ep-panel-body">
            ...
            <div class="ep-row-2">
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('timer.field.interval') }}</label>
                <div class="interval-row">
                  <input v-model.number="editTimer.interval_sec" type="number" min="30" class="ep-field-input" />
                  <span class="ep-field-hint">{{ t('timer.field.interval_hint') }} · {{ fmtInterval(editTimer.interval_sec ?? 300) }}</span>
                </div>
              </div>
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('timer.field.min_msgs') }} <span class="ep-field-hint">{{ t('timer.field.min_msgs_hint') }}</span></label>
            ...
            <div class="ep-row-3">
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('timer.field.active_when') }}</label>
              ...
              <div class="ep-field-group">
                <label class="ep-field-label">{{ t('timer.field.game') }} <span class="ep-field-hint">{{ t('timer.field.game_hint') }}</span></label>
                <input v-model="editTimer.required_game" class="ep-field-input" placeholder="Just Chatting" />
              </div>
            </div>

            <div class="ep-field-group">
              <label class="ep-field-label">{{ t('timer.field.condition') }} <span class="ep-field-hint">{{ t('timer.field.cond_hint') }}</span></label>
            ...
            <div class="ep-panel-footer">
              ...
              <div class="ep-footer-right">
                ...
                <button class="ep-btn-save" @click="saveTimer" :disabled="!!saving || !editTimer.name || !editTimer.response">
  ...
  <Teleport to="body">
    <div v-if="shareOpen" class="ep-modal-overlay" @click.self="shareOpen = false">
      <div class="ep-modal">
        <div class="ep-modal-title">{{ t('timer.share.title') }} <span class="ep-modal-name">{{ shareTimer }}</span></div>
        ...
        <div class="ep-modal-footer">
          <button class="ep-btn-cancel" @click="shareOpen = false">{{ t('timer.cancel') }}</button>
          <button class="ep-btn-save" @click="doShare" :disabled="shareSaving || !shareTarget">{{ shareSaving ? t('timer.share.copying') : t('timer.share.btn') }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
.timer-toggle-wrap { flex-shrink: 0; }
...
.ep-meta-pill.interval { color: #9d6cff; border-color: #9d6cff44; background: #9d6cff11; }
.ep-meta-pill.msgs     { color: #4ec9b0; border-color: #4ec9b044; background: #4ec9b011; }
.ep-meta-pill.when     { color: #e5c07b; border-color: #e5c07b44; background: #e5c07b11; }
.ep-meta-pill.game     { color: #23d18b; border-color: #23d18b44; background: #23d18b11; }
