...
const canView = computed(
  () => channelRole.value?.permissions.moderation_view ?? false,
);
const canManage = computed(
  () => channelRole.value?.permissions.moderation_manage ?? false,
);
...
interface SpamOptions {
  emote_target?: "emoji" | "7tv" | "twitch" | "all";
  ignore_7tv?: boolean;
}
...
const SPAM_TYPES = computed(() => [
  ...
  {
    value: "links",
    label: t("mod.spam.links"),
    hint: t("mod.spam.links_hint"),
  },
  {
    value: "emoji",
    label: t("mod.spam.emoji"),
    hint: t("mod.spam.emoji_hint"),
  },
  {
    value: "repeat",
    label: t("mod.spam.repeat"),
    hint: t("mod.spam.repeat_hint"),
  },
  {
    value: "flood",
    label: t("mod.spam.flood"),
    hint: t("mod.spam.flood_hint"),
  },
]);
...
function spamLabel(f: SpamFilter): { name: string; detail: string } {
  const name =
    SPAM_TYPES.value.find((s) => s.value === f.type)?.label ?? f.type;
  ...
  if (f.type === "caps") {
    const minPart =
      m > 0 ? `min. ${mStr} ${t("mod.spam.min_letters_hint")} ${and} ` : "";
...
async function addBlockedTerm() {
  ...
  try {
    const res = await fetch(
      `${API}/moderation/${session.value.channel}/blocked-terms`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          action: newTermAction.value,
          duration: newTermDur.value,
          is_regex: newTermIsRegex.value ? 1 : 0,
        }),
      },
    );
    ...
    newTerm.value = "";
    newTermIsRegex.value = false;
    showSuccess(t("mod.success.added"));
  } catch {
    error.value = t("mod.error.add_term");
  }
  saving.value = false;
}
...
async function removeBlockedTerm(id: number) {
  ...
  try {
    await fetch(
      `${API}/moderation/${session.value.channel}/blocked-terms/${id}`,
      {
        method: "DELETE",
    ...
    error.value = t("mod.error.remove");
  }
}
...
async function addSpamFilter() {
  ...
  try {
    const res = await fetch(
      `${API}/moderation/${session.value.channel}/spam-filters`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${session.value.token}`,
        ...
        body: JSON.stringify({
          ...
          options: JSON.stringify({
            ...(newSpamType.value === "emoji"
              ? { emote_target: newSpamEmoteTarget.value }
              : {}),
            ...(newSpamType.value === "caps"
              ? { ignore_7tv: newSpamIgnore7tv.value }
              : {}),
          }),
    ...
    error.value = t("mod.error.add_spam");
  }
  saving.value = false;
}
...
async function removeSpamFilter(id: number) {
  ...
  try {
    await fetch(
      `${API}/moderation/${session.value.channel}/spam-filters/${id}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${session.value.token}` },
      },
...
async function addNuke() {
  ...
  try {
    const res = await fetch(
      `${API}/moderation/${session.value.channel}/nukes`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${session.value.token}`,
        ...
        body: JSON.stringify({
          ...
          expires_at:
            newNukeStayActive.value && newNukeExpiry.value
              ? Date.now() + newNukeExpiryMins.value * 60_000
              : null,
        }),
      },
    );
    ...
    newNuke.value = "";
    newNukeLabel.value = "";
    newNukeStayActive.value = false;
    newNukeMatchExact.value = false;
    newNukeIsRegex.value = false;
    showSuccess(t("mod.success.nuke_created"));
  } catch {
    error.value = t("mod.error.create_nuke");
  }
  saving.value = false;
}
...
async function setNukeExpiry(nuke: NukeConfig, mins: number | null) {
  ...
  try {
    ...
    showSuccess(
      mins
        ? `${t("mod.nuke.expire_in")} ${mins} ${t("mod.nuke.min")}.`
        : t("mod.nuke.expiry_cleared"),
    );
  } catch {
    error.value = t("mod.error.set_expiry");
  }
}
...
async function fireNuke(id: number) {
  if (nukeConfirm.value !== id) {
    ...
    setTimeout(() => {
      if (nukeConfirm.value === id) nukeConfirm.value = null;
    }, 4000);
    return;
  }
  ...
  try {
    const res = await fetch(
      `${API}/moderation/${session.value.channel}/nukes/${id}/fire?lookback=${lookback}`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${session.value.token}` },
      },
    ...
    showSuccess(
      `${t("mod.nuke.fired")} ${data.affected ?? 0} ${t("mod.nuke.users_timed_out")}`,
    );
  } catch {
    error.value = t("mod.error.fire_nuke");
  }
}
...
<template>
  <div class="mod-view">
    <div class="mod-header">
      ...
      <div class="mod-sub">
        {{ t("mod.sub") }} <span class="chan">#{{ session?.channel }}</span>
      </div>
    </div>
    ...
    <div class="tabs">
      <button
        v-for="tab in ['blocked', 'spam', 'nukes'] as Tab[]"
        :key="tab"
        class="tab-btn"
    ...
    <template v-else-if="activeTab === 'blocked'">
      <div v-if="canManage" class="add-row">
        <input
          ...
          :placeholder="
            newTermIsRegex ? t('mod.field.term_re') : t('mod.field.term')
          "
          @keydown.enter="addBlockedTerm"
        />
        <label class="toggle-label">
          <input type="checkbox" v-model="newTermIsRegex" class="toggle-cb" />
          <span class="toggle-track" :class="{ on: newTermIsRegex }"
        ...
        <input
          v-if="newTermAction !== 'delete'"
          v-model.number="newTermDur"
          type="number"
          min="1"
          class="field-input dur-input"
          :placeholder="t('mod.field.secs')"
        />
        <button class="add-btn" @click="addBlockedTerm" :disabled="saving">
          {{ t("mod.add") }}
        </button>
      </div>
      <div v-if="!blockedTerms.length" class="mod-empty">
        {{ t("mod.empty.blocked") }}
      </div>
      <div v-else class="item-list">
        <div v-for="term in blockedTerms" :key="term.id" class="item-row">
          <span class="item-badge regex-badge" v-if="term.is_regex">{{
            t("mod.badge.regex")
          }}</span>
          <span class="item-term">{{ term.term }}</span>
          <span
            class="item-action"
            :style="{ color: ACTION_COLORS[term.action] }"
            >{{
              term.action === "delete"
                ? t("mod.action.delete")
                : term.action === "timeout"
          ...
          >
          <span v-if="term.action !== 'delete'" class="item-dur">{{
            fmtDur(term.duration)
          }}</span>
          <button
            v-if="canManage"
            class="item-del"
            @click="removeBlockedTerm(term.id)"
          >
            ✕
          </button>
        </div>
    ...
    <template v-else-if="activeTab === 'spam'">
      <div v-if="canManage" class="add-row">
        <select v-model="newSpamType" class="field-select flex1">
          <option v-for="s in SPAM_TYPES" :key="s.value" :value="s.value">
            {{ s.label }}
          </option>
        </select>
        ...
        <template v-if="newSpamType === 'caps'">
          <label class="toggle-label" style="gap: 6px; cursor: pointer">
            <input
              type="checkbox"
              v-model="newSpamIgnore7tv"
              class="toggle-cb"
            />
            <span class="toggle-track" :class="{ on: newSpamIgnore7tv }"
              ><span class="toggle-thumb"></span
            ></span>
        ...
        >
          <div class="threshold-sentence">
            ...
            <input
              v-model.number="newSpamMinLetters"
              type="number"
              min="0"
              max="999"
              class="field-input dur-input"
            ...
            />
            <span class="ts-lbl"
              >{{ t("mod.spam.min_letters_hint") }}
              {{ t("mod.spam.and") }} ≥</span
            >
            <input
              v-model.number="newSpamThreshold"
              type="number"
              min="1"
              max="9999"
              class="field-input dur-input"
            />
            <span class="ts-lbl">{{
              SPAM_TYPES.find((s) => s.value === newSpamType)?.hint
            }}</span>
          </div>
        </template>
        ...
        <input
          v-if="newSpamAction !== 'delete'"
          v-model.number="newSpamDur"
          type="number"
          min="1"
          class="field-input dur-input"
          :placeholder="t('mod.field.secs')"
        />
        <button class="add-btn" @click="addSpamFilter" :disabled="saving">
          {{ t("mod.add") }}
        </button>
      </div>
      <div v-if="!spamFilters.length" class="mod-empty">
        {{ t("mod.empty.spam") }}
      </div>
      <div v-else class="item-list">
        <div v-for="f in spamFilters" :key="f.id" class="item-row">
          ...
          <span
            class="item-action"
            :style="{ color: ACTION_COLORS[f.action] }"
            >{{
              f.action === "delete"
                ? t("mod.action.delete")
                : f.action === "timeout"
          ...
          >
          <span v-if="f.action !== 'delete'" class="item-dur">{{
            fmtDur(f.duration)
          }}</span>
          <button
            v-if="canManage"
            class="item-del"
            @click="removeSpamFilter(f.id)"
          >
            ✕
          </button>
        </div>
    ...
    <template v-else-if="activeTab === 'nukes'">
      ...
      <div v-if="canManage" class="add-row nuke-add-row">
        <div class="nuke-inputs-top">
          <input
            ...
            :placeholder="
              newNukeIsRegex
                ? t('mod.nuke.trigger_re_ph')
                : t('mod.nuke.trigger_ph')
            "
            @keydown.enter="addNuke"
          />
          <input
            v-model="newNukeLabel"
            class="field-input"
            style="width: 120px"
            :placeholder="t('mod.nuke.label')"
          />
          <div class="threshold-wrap">
            <span class="threshold-lbl">{{ t("mod.nuke.timeout") }}</span>
            <input
              v-model.number="newNukeDur"
              type="number"
              min="1"
              class="field-input dur-input"
            />
          ...
          <div class="threshold-wrap">
            ...
            <input
              v-model.number="newNukeLookback"
              type="number"
              min="1"
              max="1440"
              class="field-input dur-input"
        ...
        <div class="nuke-toggles-row">
          <label class="toggle-label">
            <input
              type="checkbox"
              v-model="newNukeStayActive"
              class="toggle-cb"
            />
            <span class="toggle-track" :class="{ on: newNukeStayActive }"
              ><span class="toggle-thumb"></span
            ></span>
          ...
          <label class="toggle-label" :class="{ dimmed: newNukeIsRegex }">
            <input
              type="checkbox"
              v-model="newNukeMatchExact"
              class="toggle-cb"
              :disabled="newNukeIsRegex"
            />
            <span
              class="toggle-track"
              :class="{ on: newNukeMatchExact && !newNukeIsRegex }"
              ><span class="toggle-thumb"></span
            ></span>
          ...
          <label class="toggle-label">
            <input
              type="checkbox"
              v-model="newNukeIsRegex"
              class="toggle-cb"
              @change="newNukeIsRegex && (newNukeMatchExact = false)"
            />
            <span class="toggle-track" :class="{ on: newNukeIsRegex }"
              ><span class="toggle-thumb"></span
            ></span>
            <span class="toggle-text">{{ t("mod.nuke.regex") }}</span>
            <span class="info-icon" :title="t('mod.nuke.regex_hint')">ⓘ</span>
          ...
          <label class="toggle-label" :class="{ dimmed: !newNukeStayActive }">
            <input
              type="checkbox"
              v-model="newNukeExpiry"
              class="toggle-cb"
              :disabled="!newNukeStayActive"
            />
            <span
              class="toggle-track"
              :class="{ on: newNukeExpiry && newNukeStayActive }"
              ><span class="toggle-thumb"></span
            ></span>
          ...
          <div v-if="newNukeExpiry && newNukeStayActive" class="threshold-wrap">
            <input
              v-model.number="newNukeExpiryMins"
              type="number"
              min="1"
              class="field-input dur-input"
            />
      ...
      <div v-if="!nukes.length" class="mod-empty">
        {{ t("mod.empty.nukes") }}
      </div>
      <div v-else class="item-list">
        <div v-for="n in nukes" :key="n.id" class="item-row">
          <div class="nuke-row-badges">
            <span
              v-if="n.stay_active"
              class="item-badge stay-badge"
              :title="t('mod.nuke.stay_hint')"
              >{{ t("mod.badge.stay") }}</span
            >
            <span
              v-if="n.is_regex"
              class="item-badge regex-badge"
              :title="t('mod.nuke.regex_hint')"
              >{{ t("mod.badge.regex") }}</span
            >
            <span
              v-if="n.match_exact"
              class="item-badge exact-badge"
              :title="t('mod.nuke.exact_hint')"
          ...
          <div class="threshold-wrap" style="margin-left: auto">
            ...
            <input
              ...
              @input="
                nukeLookbackOverride[n.id] = parseInt(
                  ($event.target as HTMLInputElement).value,
                )
              "
              class="field-input dur-input"
              style="width: 52px"
              :title="t('mod.nuke.lookback')"
            />
            <span class="threshold-hint">{{ t("mod.nuke.min") }}</span>
          </div>
          ...
          <div v-if="n.stay_active" class="expiry-wrap">
            ...
            >
              {{
                nukeExpiresIn(n) === t("mod.nuke.expired")
                  ? t("mod.nuke.expired")
                  : `${t("mod.nuke.expires")} ${nukeExpiresIn(n)}`
              }}
              <button
                v-if="canManage"
                class="expiry-clear"
                @click="setNukeExpiry(n, null)"
                title="✕"
              >
                ✕
              </button>
          ...
          <button
            v-if="canManage"
            class="nuke-fire-btn"
            :class="{ confirm: nukeConfirm === n.id }"
            @click="fireNuke(n.id)"
          >
            {{ nukeConfirm === n.id ? t("mod.nuke.sure") : t("mod.nuke.fire") }}
          </button>
          <button v-if="canManage" class="item-del" @click="removeNuke(n.id)">
            ✕
          </button>
        </div>
      </div>
...
.toggle-track.on {
  background: rgba(111, 43, 255, 0.5);
