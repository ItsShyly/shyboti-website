...
async function createNote() {
  ...
  creating.value = true; createError.value = ''; lastLink.value = ''
  try {
    const headers: Record<string, string> = { 'Content-Type': 'application/json' }
    if (session.value) headers['Authorization'] = `Bearer ${session.value.token}`
    // Clamp expiry for guests
    const effectiveExpiry = (isGuest.value && expiry.value === 'forever') ? '30d' : expiry.value
    const res = await fetch(`${API}/notes/create`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ content: noteText.value, expiry: effectiveExpiry }),
    })
    if (!res.ok) { const d = await res.json() as any; throw new Error(d.error ?? 'Create failed') }
    const data = await res.json() as { id: string }
    lastLink.value = noteUrl(data.id)
  } catch (e: any) { createError.value = e.message ?? 'Could not create note' }
...
function newNote() {
  lastLink.value = ''; createError.value = ''
  noteText.value = ''
...
async function deleteNote(id: string) {
  if (deleteId.value !== id) {
    deleteId.value = id
    setTimeout(() => { if (deleteId.value === id) deleteId.value = null }, 3000)
    return
  }
  deleteId.value = null
  try {
  ...
  <div class="notes-page">
    ...
    <div v-if="view === 'list'" class="list-view">
      <div class="list-header">
        <button class="back-btn" @click="view = 'write'">{{ t('notes.back') }}</button>
        <div class="list-title">{{ t('notes.your') }}</div>
        <span class="list-count" v-if="notes.length">{{ notes.length }} / 100</span>
      </div>
      ...
      <div v-else class="notes-list">
        <div v-for="note in notes" :key="note.id" class="note-item" :class="{ 'is-expired': note.expires_at && note.expires_at < Date.now() }">
          ...
          <div class="note-meta">
            ...
            <span class="note-dot">·</span>
            <span class="note-expiry" :class="{ expired: note.expires_at && note.expires_at < Date.now() }">
      ...
      <template v-else>
        ...
        <div class="bottom-bar">
          <div class="bottom-left">
            <span class="char-count">{{ charCount.toLocaleString() }} {{ t('notes.chars') }}</span>
          ...
          <div class="bottom-right">
            ...
            <button class="create-btn" @click="createNote" :disabled="creating || !noteText.trim()">
...
<style scoped>
.notes-page { display: flex; flex-direction: column; height: 100%; min-height: 0; }
...
/* >>> Write view <<< */
.write-view { display: flex; flex-direction: column; flex: 1; min-height: 0; }

.note-editor {
  flex: 1; resize: none; background: #111217; border: 1px solid #1e1e24;
  border-bottom: none; color: #e0e0e0; font-family: 'Consolas','Fira Mono',monospace;
  font-size: 13px; line-height: 1.7; padding: 20px 22px;
  outline: none; min-height: 200px;
}
.note-editor::placeholder { color: #2a2a35; }
.note-editor:focus { border-color: #6f2bff33; }

/* >>> Bottom bar <<< */
.bottom-bar {
  display: flex; align-items: center; justify-content: space-between; gap: 12px;
  padding: 10px 14px; background: #0d0d10;
  border: 1px solid #1e1e24; border-top: none; flex-shrink: 0; flex-wrap: wrap;
}
.bottom-left  { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
.bottom-right { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.char-count { font-size: 11px; color: #444; font-family: 'Consolas','Fira Mono',monospace; }
.expiry-row { display: flex; align-items: center; gap: 6px; }
.expiry-label { font-size: 11px; color: #555; }
.expiry-select {
  background: #111217; border: 1px solid #2a2a30; color: #aaa;
  font-family: inherit; font-size: 11px; padding: 4px 8px;
  outline: none; cursor: pointer; appearance: none;
}
.expiry-select:focus { border-color: #6f2bff55; }

.your-btn {
  display: flex; align-items: center; gap: 6px;
  height: 32px; padding: 0 12px; border: 1px solid #2a2a30;
...
.your-icon { width: 13px; height: 13px; flex-shrink: 0; }
.create-btn {
...
.create-btn:hover:not(:disabled) { background: #7f3fff; }
.create-btn:disabled { opacity: .4; cursor: not-allowed; }

.create-error {
  padding: 7px 14px; font-size: 11px; color: #f14949;
  background: rgba(241,73,73,.08); border: 1px solid #f1494933; border-top: none;
  flex-shrink: 0;
}
...
.result-box {
  ...
  background: #141418; border: 1px solid #2a2a30; padding: 32px 40px; max-width: 500px; width: 100%;
}
.result-label { font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: .05em; align-self: flex-start; }
.result-link-row { display: flex; align-items: center; gap: 8px; width: 100%; }
.result-code {
  flex: 1; font-family: 'Consolas','Fira Mono',monospace; font-size: 13px;
  color: #9d6cff; background: #0d0d10; border: 1px solid #2a2a30;
  padding: 8px 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.copy-btn {
  height: 34px; padding: 0 14px; border: 1px solid #6f2bff55;
  background: #6f2bff18; color: #9d6cff; font-family: inherit; font-size: 11px;
  cursor: pointer; flex-shrink: 0; transition: background .15s; white-space: nowrap;
}
.copy-btn:hover { background: #6f2bff30; }
.new-note-btn {
  height: 30px; padding: 0 14px; border: 1px solid #2a2a30;
  background: transparent; color: #666; font-family: inherit; font-size: 11px;
  cursor: pointer; align-self: flex-start; transition: all .15s;
}
.new-note-btn:hover { color: #e0e0e0; border-color: #444; }

/* >>> List view <<< */
.list-view { display: flex; flex-direction: column; height: 100%; min-height: 0; }
.list-header {
  display: flex; align-items: center; gap: 12px;
  padding: 0 0 14px; border-bottom: 1px solid #222; flex-shrink: 0; margin-bottom: 16px;
}
.back-btn {
  height: 28px; padding: 0 10px; border: 1px solid #2a2a30;
  background: transparent; color: #666; font-family: inherit; font-size: 11px; cursor: pointer;
}
.back-btn:hover { color: #e0e0e0; border-color: #444; }
.list-title { font-size: 16px; font-weight: 700; color: #e0e0e0; flex: 1; }
.list-count { font-size: 11px; color: #555; }
.list-empty { color: #444; font-size: 13px; padding: 40px; text-align: center; }

.notes-list { display: flex; flex-direction: column; gap: 2px; overflow-y: auto; flex: 1; }
.note-item {
  display: flex; flex-direction: column; gap: 4px;
  padding: 12px 14px; background: #141418; border-bottom: 1px solid #1a1a1e;
  transition: background .1s;
}
.note-item:hover { background: #1c1c20; }

.note-preview {
  font-family: 'Consolas','Fira Mono',monospace; font-size: 12px; color: #ccc;
  text-decoration: none; display: block;
  overflow: hidden; white-space: nowrap; text-overflow: ellipsis;
  transition: color .1s;
}
.note-preview:hover { color: #9d6cff; }

.note-meta {
  display: flex; align-items: center; gap: 6px;
  font-size: 10px; color: #444;
}
.note-dot { color: #2a2a30; }
.note-expiry.expired { color: #f14949; }

.note-actions { display: flex; align-items: center; gap: 6px; }
.note-link-code { font-family: 'Consolas','Fira Mono',monospace; font-size: 10px; color: #9d6cff; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.copy-btn-sm {
  height: 20px; padding: 0 7px; border: 1px solid #6f2bff44;
  background: transparent; color: #9d6cff; font-family: inherit; font-size: 9px;
  cursor: pointer; flex-shrink: 0;
}
.copy-btn-sm:hover { background: #6f2bff18; }
.delete-btn-sm {
  width: 22px; height: 22px; border: 1px solid #f1494933;
  background: transparent; color: #f14949; font-size: 11px;
