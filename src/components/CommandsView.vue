  ...
  <div class="cmd-root">
    <div class="cmd-tabs">
      <button class="cmd-tab" :class="{ active: activeTab === 'Default' }" @click="activeTab = 'Default'">{{
        t('cmd.title_default') }}</button>
      <button class="cmd-tab" :class="{ active: activeTab === 'Custom' }" @click="activeTab = 'Custom'">{{
        t('cmd.title_custom') }}</button>
      <button class="cmd-tab" :class="{ active: activeTab === 'Extras' }" @click="activeTab = 'Extras'">{{
        t('cmd.title_extras') }}</button>
      <button class="cmd-tab reload-tab" @click="reloadAll" :disabled="reloading" title="Reload">{{ reloading ? '…' :
        '↺' }}</button>
    </div>
            ...
            <div class="table-row" :class="{ saving: saving === cmd.name, expanded: expandedDefault.has(cmd.name) }">
              ...
              <div class="row-chevron-cell">
                <button v-if="cmd.argVariants?.length" class="row-chevron"
                  ...
                  title="Show argument variants">▾</button>
              </div>
              ...
              <div>
                <button class="access-btn"
                  :class="{ 'access-mod': cmd.modOnly, 'access-bc': cmd.broadcasterOnly, disabled: !canToggle }"
                  @click="cycleRestriction(cmd)">{{ restrictionLabel(cmd) }}</button>
              </div>

              <div>
                <div class="cd-input-wrap" :class="{ disabled: !canEdit }">
                  <input type="number" min="0" class="cd-input" :disabled="!canEdit" :value="cmd.cooldown"
              ...
              <div>
                <button class="edit-btn" :class="{ blocked: BLOCKED.includes(cmd.name) || !canEdit }"
    ...
    <template v-if="activeTab === 'Custom'">
      <div class="custom-header">
        <div class="custom-header-left">
          <span class="custom-count">{{ customCommands.length }} {{ customCommands.length !== 1 ? t('cmd.count_plural')
            : t('cmd.count') }}</span>
          ...
          <button v-else class="sync-config-btn" @click="syncOpen = !syncOpen">{{ t('cmd.sync.config') }} <span
              class="sync-chevron">{{ syncOpen ? '▲' : '▼' }}</span></button>
        </div>
        ...
        <div v-else class="new-cmd-row">
          ...
          <input ref="newCmdInput" v-model="newCmdName" class="new-cmd-input"
      ...
      <div v-if="syncOpen" class="sync-panel">
        <div class="sync-row">
          ...
          <button class="sync-save-btn" @click="saveSync" :disabled="syncSaving || !syncFrom">{{ syncSaving ? '…' :
            syncConf?.is_active ? t('cmd.sync.update') : t('cmd.sync.enable') }}</button>
          <button v-if="syncConf?.is_active" class="sync-run-btn" @click="runSync" :disabled="syncRunning">{{
            syncRunning ? '…' : t('cmd.sync.pull') }}</button>
          <button v-if="syncConf?.is_active" class="sync-stop-btn" @click="stopSync">{{ t('cmd.sync.stop') }}</button>
        </div>
        <div v-if="syncConf?.last_synced" class="sync-last">{{ t('cmd.sync.last') }} {{ new
          Date(syncConf.last_synced).toLocaleString() }}</div>
        <div v-if="syncMsg" class="sync-msg" :class="{ err: syncMsg.includes('fail') || syncMsg.includes('Error') }">{{
          syncMsg }}</div>
      </div>
            ...
            <div class="table-row custom-row" :class="{ expanded: expandedCustom.has(cmd.name) }">
              ...
              <div class="row-chevron-cell">
                <button v-if="customHasArgs(cmd)" class="row-chevron" :class="{ open: expandedCustom.has(cmd.name) }"
                  @click.stop="toggleExpandCustom(cmd.name)" title="Show argument variants">▾</button>
              </div>
              ...
              <div class="custom-actions">
                <button class="edit-btn" :class="{ blocked: !canEdit }" @click="canEdit && openEdit(cmd.name, false)">{{
                  canEdit ? t('cmd.edit') : t('cmd.view') }}</button>
                <button class="share-btn" @click="openShare(cmd.name)" title="Copy to another channel">↪</button>
                <button v-if="canDelete" class="del-btn"
                  :class="{ confirm: deleteConfirmName === cmd.name, deleting: deletingName === cmd.name }"
                  @click="deleteCustom(cmd.name)"
                  :title="deleteConfirmName === cmd.name ? 'Click again to confirm' : 'Delete'">{{ deletingName ===
                    cmd.name ? '…' : deleteConfirmName === cmd.name ? t('cmd.delete_sure') : t('cmd.delete') }}</button>
              </div>
            </div>
    ...
    <div v-if="shareOpen" class="modal-overlay" @click.self="shareOpen = false">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          <button class="btn-save" @click="doShare" :disabled="shareSaving || !shareTarget">{{ shareSaving ?
            t('cmd.share.copying') : t('cmd.share.btn') }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
.access-btn.access-mod {
  ...
  color: #c792ea;
  background: rgba(199, 146, 234, .08);
}

.access-btn.access-mod:hover {
  background: rgba(199, 146, 234, .15);
}

.access-btn.access-bc {
  border-color: #f1494955;
  color: #f14949;
  background: rgba(241, 73, 73, .08);
}
...
.extras-save-btn:hover:not(:disabled) {
  background: #7f3fff;
}

.extras-save-btn:disabled {
  opacity: .5;
  cursor: not-allowed;
}

.extras-save-btn.saved {
  background: #1a3d2a;
  color: #23d18b;
  cursor: default;
}
