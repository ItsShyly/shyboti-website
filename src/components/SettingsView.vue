...
interface SceneBinding  { command: string; scene: string }
interface SourceBinding { command: string; source: string; action: string; value?: number }
const OBS_PW_MASK = '••••••••'
const OBS_ACTIONS = ['show', 'hide', 'toggle', 'mute', 'unmute', 'mutetoggle', 'volume']

const obsHost         = ref('127.0.0.1')
const obsPort         = ref(4455)
const obsPassword     = ref('')
const obsEnabled      = ref(false)
const obsLoading      = ref(false)
const obsSaving       = ref(false)
const obsMsg          = ref('')
const obsErr          = ref('')
const obsStatus       = ref<ObsStatus>({ state: 'disconnected', error: '', currentScene: '', scenes: [] })
const obsTesting      = ref(false)
const obsTestResult   = ref<{ ok: boolean; error?: string; scenes?: ObsScene[]; currentScene?: string } | null>(null)
const sceneBindings   = ref<SceneBinding[]>([])
const sourceBindings  = ref<SourceBinding[]>([])
const obsSources        = ref<{ sourceName: string; isAudioSource: boolean }[]>([])
const obsSourcesScene   = ref('')
const obsSourcesLoading = ref(false)
...
async function loadObs() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      obsHost.value     = d.ws_host
      obsPort.value     = d.ws_port
      obsPassword.value = d.has_password ? OBS_PW_MASK : ''
      obsEnabled.value  = d.enabled
      obsStatus.value   = d.status
...
async function saveObsConnection(partial: Record<string, any>) {
...
function saveObsFields() {
  saveObsConnection({ ws_host: obsHost.value.trim(), ws_port: Number(obsPort.value) || 4455, ws_password: obsPassword.value })
}

function toggleObsEnabled() {
  obsEnabled.value = !obsEnabled.value
  saveObsConnection({ enabled: obsEnabled.value })
}
...
async function testObs() {
  ...
  try {
    ...
    obsTestResult.value = await res.json()
    if (obsTestResult.value?.ok && obsTestResult.value.scenes) obsStatus.value.scenes = obsTestResult.value.scenes
  } catch { obsTestResult.value = { ok: false, error: 'Request failed' } }
  obsTesting.value = false
}

function addSceneBinding()          { sceneBindings.value.push({ command: '', scene: obsStatus.value.scenes[0]?.sceneName ?? '' }) }
function removeSceneBinding(i: number)  { sceneBindings.value.splice(i, 1) }
function addSourceBinding()         { sourceBindings.value.push({ command: '', source: '', action: 'toggle' }) }
function removeSourceBinding(i: number) { sourceBindings.value.splice(i, 1) }
...
async function saveObsBindings() {
  ...
  try {
    const res = await fetch(`${API}/obs-control/${session.value.channel}/bindings`, {
      ...
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.value.token}` },
      body: JSON.stringify({
        scene_bindings:  sceneBindings.value.filter(b => b.command.trim() && b.scene.trim()),
        source_bindings: sourceBindings.value.filter(b => b.command.trim() && b.source.trim()),
      })
    })
    if (!res.ok) throw new Error()
    obsMsg.value = 'Bindings saved!'; setTimeout(() => obsMsg.value = '', 2000)
  } catch { obsErr.value = 'Could not save bindings.' }
  obsSaving.value = false
}

async function loadObsSources(sceneName: string) {
  ...
  try {
    ...
    else obsSources.value = []
  } catch { obsSources.value = [] }
  obsSourcesLoading.value = false
}
...
async function loadAll() { await load(); await load7tvSet(); await loadObs() }
onMounted(loadAll)
  ...
  <div class="settings">
    ...
    <div class="cards-grid">
      ...
      <div class="card card-wide" v-if="isBroadcaster">
        ...
        <div class="card-body">
          <div v-if="obsLoading" class="card-loading">Loading...</div>
          <template v-else>

            <div class="toggle-row">
              <div class="status-dot" :class="{ active: obsStatus.state === 'connected' }"></div>
              <span class="status-text">
                {{ obsStatus.state === 'connected' ? `Connected · scene: ${obsStatus.currentScene}` :
                   obsStatus.state === 'connecting' ? 'Connecting...' :
                   obsStatus.state === 'error' ? `Error: ${obsStatus.error}` : 'Disconnected' }}
              </span>
              <div class="spacer"></div>
              <span class="status-badge" :class="obsEnabled ? 'badge-on' : 'badge-off'">{{ obsEnabled ? 'ON' : 'OFF' }}</span>
            </div>

            <div class="section-note">
              Host should be a <strong>Tailscale IP</strong> (<code class="code">100.x.y.z</code>) for the machine running OBS,
              not a public IP or hostname - install Tailscale on that PC and join it to the bot's tailnet first.
            </div>

            <div class="emote-row">
              <span class="emote-lbl">Host</span>
              <input v-model="obsHost" class="field-sm" placeholder="100.x.y.z" spellcheck="false" />
            </div>
            <div class="emote-row">
              <span class="emote-lbl">Port</span>
              <input v-model.number="obsPort" type="number" class="field-sm" placeholder="4455" />
            </div>
            <div class="emote-row">
              <span class="emote-lbl">Password</span>
              <input v-model="obsPassword" type="password" class="field-sm" placeholder="OBS-websocket server password"
                @focus="obsPassword === '••••••••' && (obsPassword = '')" />
            </div>

            <div class="emote-row">
              <button class="fetch-btn" :disabled="obsTesting || !obsHost.trim()" @click="testObs">
                {{ obsTesting ? 'Testing...' : 'Test Connection' }}
              </button>
              <button class="toggle-btn" :class="{ 'toggle-btn-on': obsEnabled }" :disabled="obsSaving" @click="toggleObsEnabled">
                {{ obsEnabled ? 'Disable' : 'Enable' }}
              </button>
              <button class="fetch-btn" :disabled="obsSaving" @click="saveObsFields">Save</button>
            </div>

            <div v-if="obsTestResult" class="card-msg" :class="obsTestResult.ok ? 'ok' : 'err'">
              <template v-if="obsTestResult.ok">&#10003; Connected · {{ obsTestResult.scenes?.length ?? 0 }} scene(s) found · current: {{ obsTestResult.currentScene }}</template>
              <template v-else>{{ obsTestResult.error }}</template>
            </div>

            <template v-if="obsStatus.scenes.length">
              <div class="style-title" style="margin-top: 4px;">Scene Bindings</div>
              <div v-for="(b, i) in sceneBindings" :key="'sb'+i" class="emote-row">
                <input v-model="b.command" class="field-sm" style="max-width: 130px;" placeholder="chat command" />
                <select v-model="b.scene" class="field-sm">
                  <option v-for="s in obsStatus.scenes" :key="s.sceneName" :value="s.sceneName">{{ s.sceneName }}</option>
                </select>
                <button class="danger-sm" @click="removeSceneBinding(i)">Remove</button>
              </div>
              <div class="emote-row">
                <button class="fetch-btn" @click="addSceneBinding">+ Add scene binding</button>
              </div>

              <div class="style-title" style="margin-top: 4px;">Source Bindings</div>
              <div class="emote-row">
                <span class="emote-lbl">Load sources from</span>
                <select class="field-sm" :value="obsSourcesScene" @change="loadObsSources(($event.target as HTMLSelectElement).value)">
                  <option value="" disabled>select a scene...</option>
                  <option v-for="s in obsStatus.scenes" :key="s.sceneName" :value="s.sceneName">{{ s.sceneName }}</option>
                </select>
                <span v-if="obsSourcesLoading" class="card-loading">loading...</span>
              </div>
              <datalist id="obs-source-list">
                <option v-for="s in obsSources" :key="s.sourceName" :value="s.sourceName" />
              </datalist>
              <div v-for="(b, i) in sourceBindings" :key="'sob'+i" class="emote-row">
                <input v-model="b.command" class="field-sm" style="max-width: 110px;" placeholder="chat command" />
                <input v-model="b.source" class="field-sm" list="obs-source-list" style="max-width: 140px;" placeholder="source name" />
                <select v-model="b.action" class="field-sm">
                  <option v-for="a in OBS_ACTIONS" :key="a" :value="a">{{ a }}</option>
                </select>
                <input v-if="b.action === 'volume'" v-model.number="b.value" type="number" min="0" max="100" class="field-sm" style="max-width: 70px;" placeholder="%" />
                <button class="danger-sm" @click="removeSourceBinding(i)">Remove</button>
              </div>
              <div class="emote-row">
                <button class="fetch-btn" @click="addSourceBinding">+ Add source binding</button>
              </div>

              <div class="emote-row">
                <button class="save-btn" :disabled="obsSaving" @click="saveObsBindings">{{ obsSaving ? 'Saving...' : 'Save Bindings' }}</button>
              </div>
            </template>
            <div v-else class="section-note">
              Run "Test Connection" (or enable + wait for it to connect) to load your scene list before adding bindings.
            </div>

            <div v-if="obsMsg" class="card-msg ok">{{ obsMsg }}</div>
            <div v-if="obsErr" class="card-msg err">{{ obsErr }}</div>
          </template>
        </div>
      </div>
